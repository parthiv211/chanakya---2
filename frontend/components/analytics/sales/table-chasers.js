...
function convertObject(oldObject) {
  ...
  for (let key of Object.keys(oldObject)) {
    ...
    // if key is qty_sales then sort it from high to low by default
    if (key === "qty_sales") {
    }
...
export default function TableChasers() {
  const filteredData = filterArrayByValues(data.info.details, "grade", ["B"]);
