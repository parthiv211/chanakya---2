...
export default function Sales() {
  ...
  const [sortedData, setSortedData] = useState(
    sortObjectByValues(data.info.soh[0], excludeKeys)
  );

  const totalSoh = sumObjectValues(data.info.soh[0], excludeKeys);
          ...
          <div className="col-span-12 row-span-4 flex h-full flex-col border p-4">
            ...
            <div className="flex items-center">
              <div className="mr-4 flex flex-col">
                <span className="text-sm text-slate-700">Total SOH</span>
                <span className="text-2xl text-slate-700">
                  {formatNumberToIn(totalSoh)}
                </span>
              </div>
            </div>
